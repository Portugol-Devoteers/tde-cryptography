...
// Função para mover os elementos de uma matriz 4x4 uma posição para a esquerda
void MoverM(unsigned char matriz[4][4]) {
    // Matriz temporária para armazenar os elementos movidos
    ...
    // Mova os elementos uma posição para a esquerda
    for (int i = 0; i < 4; i++) {
        for (int j = 0; j < 4; j++) {
            if (i == 3){
                k = 0;
...
void SBox (unsigned char matriz[4][4]) {
    ...
    for (int i = 0; i < 4; i++) {
        for (int j = 0; j < 4; j++) {
            ...
            matriz [i][j] = sBox[(valor >> 4) & 0x0F][valor & 0x0F];
...
int main() {
    ...
    if (len < 16) {
        ...
        printf("A chave digitada tem mais de 16 caracteres e e maior do que o permitido.\n");
        return 1; // Saída com erro
    }
    
    chaveParaMatriz(chave, M);
    
    // Chame a função de impressão da matriz
    printMatriz(M);

    // Agora, imprima os dois dígitos de M[0][0] separadamente
    unsigned char valor = M[0][0];
    printf("Digitos de M[0][0]: %02X %02X\n", (valor >> 4) & 0x0F, valor & 0x0F);

    MoverM(M);

    printf("\nMatriz movida uma posicao para a direita:\n");
    printMatriz(M);
    SBox (M);
    printf("\nMatriz trocada com a SBOX:\n");
    printMatriz(M);
